const x="https://support-tickets-api.dopetone701.workers.dev",w="https://emails-api.dopetone701.workers.dev",E=new URL("/public/images/logo.png",window.location.origin).href;new URL("/public/images/dt-boss.png",window.location.origin).href;new URL("/public/images/dt-boss-logo.png",window.location.origin).href;new URL("/public/images/default-user.png",window.location.origin).href;new URL("/public/audio/dt-pro-tag.wav",window.location.origin).href;function y(){return`
<div class="wrap" style="min-width:0; overflow-wrap:anywhere;">
  <div class="topbar" style="min-width:0; margin-bottom:16px;">
    <a href="#/home" data-link class="logo-link" style="display:flex;align-items:center;gap:10px;text-decoration:none;">
      <img src="${E}" alt="logo" style="width:36px;height:36px;object-fit:contain;" onerror="this.style.display='none'">
...
`}function f(){const g={"Order / Delivery":"Got it! Checking your order delivery. Update in email in under 1 hour.","License Question":"License question received! Team will email you details shortly.","Payment Issue":"Payment issue noted! Looking into it now.","File / Stems Issue":"File issue received! Fresh link coming to email soon.","Custom Beat":"Custom beat request! Love it. Replying with next steps.",Other:"Message sent! Reply in under 1 hour."};document.querySelectorAll(".acc h4").forEach(function(t){t.style.cursor="pointer",t.onclick=function(){const i=t.parentElement,a=i.classList.contains("open");if(document.querySelectorAll(".acc").forEach(function(o){o.classList.remove("open");const s=o.querySelector(".ans");s&&(s.style.display="none");const e=o.querySelector(".plus");e&&(e.textContent="+")}),!a){i.classList.add("open");const o=i.querySelector(".ans");o&&(o.style.display="block");const s=t.querySelector(".plus");s&&(s.textContent="x")}}}),document.querySelectorAll("[data-scroll]").forEach(function(t){t.style.cursor="pointer",t.onclick=function(){const i=document.getElementById(t.dataset.scroll);i&&i.scrollIntoView({behavior:"smooth",block:"start"})}});const u=document.getElementById("searchInput");u&&(u.oninput=function(t){const i=t.target.value.toLowerCase();document.querySelectorAll(".acc, #licenseSection, #termsSection, #privacySection").forEach(function(a){const o=a.textContent.toLowerCase();a.style.display=o.includes(i)||i===""?"":"none"})});const n=document.getElementById("sendTicket");n&&(n.textContent="SEND MESSAGE",n.onclick=async function(){const t=document.getElementById("tName"),i=document.getElementById("tEmail"),a=document.getElementById("tOrder"),o=document.getElementById("tCat"),s=document.getElementById("tMsg"),e=document.getElementById("ticketStatus"),r=t?t.value.trim():"",l=i?i.value.trim():"",p=a?a.value.trim():"",d=o?o.value:"Other",m=s?s.value.trim():"";if(!r||!l||!m){e&&(e.style.display="block",e.style.color="#ef4444",e.textContent="Please fill name, email and your message.");return}n.disabled=!0,n.textContent="SENDING...",e&&(e.style.display="block",e.style.color="#888",e.textContent="Sending your message...");try{const c=await fetch(x+"/api/tickets/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:r,username:r,email:l,subject:d+" - Order "+(p||"no-id"),message:"Category: "+d+`
Order: `+p+`
Email: `+l+`

`+m,priority:"Medium",status:"open",source:"help_page"})}),h=await c.json();if(!c.ok||!h.success)throw new Error("Failed");const v=h.id||"";try{await fetch(w+"/api/emails/bulk",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({emails:[l],name:r,category:d,orderId:p,ticketId:v})})}catch(b){console.log("Email failed but ticket saved",b)}e&&(e.style.color="#10b981",e.innerHTML=g[d]+"<br><span style='font-size:11px;color:#666'>Email sent to "+l+"</span>"),t&&(t.value=""),i&&(i.value=""),a&&(a.value=""),s&&(s.value=""),o&&(o.selectedIndex=0)}catch(c){console.log(c),e&&(e.style.color="#ef4444",e.textContent="Could not send - WhatsApp +971524082460")}finally{n.disabled=!1,n.textContent="SEND MESSAGE"}})}const S=y,A=f,k={renderHelp:y,initHelp:f,render:S,init:A};export{k as default,A as init,f as initHelp,S as render,y as renderHelp};
