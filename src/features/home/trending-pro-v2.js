...
.spotify-play{
  ...
  background:linear-gradient(135deg,#4da6ff,#ff4d94);color:#fff;border:none;
  display:grid;place-items:center;cursor:pointer;box-shadow:0 6px 16px rgba(0,0,0,0.5);
  opacity:0;transform:translateY(8px) scale(.92);transition:.22s;z-index:2;
}
...
.eq-wrap{
  position:absolute;left:8px;bottom:8px;display:flex;gap:2px;align-items:end;
  background:rgba(0,0,0,0.6);backdrop-filter:blur(6px);padding:4px 6px;border-radius:99px;
  opacity:0;transform:translateY(6px);transition:.2s;z-index:2;
}
...
/* === OVERFLOW FIX === */
#homeSmartWrap, #homeSmartWrap.side-by-side,.ntg-shell,.ntg-card{
  overflow:visible!important;
  max-height:none!important;
  min-height:0!important;
}
#homeSmartWrap.side-by-side.ntg-card{
...
`;
  ...
  const getCardToSwap = () => {
    ...
    const eligible = cards.filter(card => {
      if (card.classList.contains("playing")) return false;
      return (now - (recentlySwapped.get(card) || 0)) > 7000;
    });
    if (eligible.length) return eligible[Math.floor(Math.random() * eligible.length)];
    return cards.filter(c =>!c.classList.contains("playing"))
     .sort((a,b)=>(recentlySwapped.get(a)||0)-(recentlySwapped.get(b)||0))[0];
  };
  ...
  const getNextBeat = () => {
    ...
    while (attempts < 2) {
      while (shuffleBag.length) {
        ...
        if (!visibleIds.has(String(candidate.id)) && String(candidate.id)!== String(activeBeatId)) {
          return candidate;
        }
      }
      refillBag();
  ...
  const swapOne = async () => {
    ...
    const card = getCardToSwap();
    if (!card) { scheduleNext(); return; }
    const beat = getNextBeat();
    if (!beat) { scheduleNext(); return; }

    const img = card.querySelector("img");
    const title = card.querySelector(".spotify-title");
    const sub = card.querySelector(".spotify-sub");
    const priceEl = card.querySelector(".spotify-price");
    ...
    const price = getPrice(beat);
    const realIdx = queue.findIndex(b => String(b.id) === String(beat.id));
    card.dataset.id = beat.id;
    card.dataset.queue = realIdx;
    img.src = src;
    title.textContent = beat.title || "Untitled";
    sub.textContent = `${beat.genre || "Trap"} • ${beat.bpm || "--"} BPM`;
    priceEl.textContent = price;
    priceEl.className = `spotify-price ${price === "FREE"? "free" : ""}`;
    recentlySwapped.set(card, Date.now());

    card.classList.remove("swap-out");
    card.classList.add("swap-in");
    setTimeout(() => card.classList.remove("swap-in"), 350);
    scheduleNext();
  };

  const scheduleNext = () => {
    window.__trendingSwap__ = setTimeout(swapOne, 3200 + Math.random() * 800);
  };

  grid.onmouseenter = () => (isHovering = true);
  grid.onmouseleave = () => (isHovering = false);
    ...
    const playAction = (e) => {
      ...
      if (String(currentId) === String(id) && window.globalPlayer) {
        if (window.globalPlayer.isPlaying) {
          ...
          window.globalPlayer.resume?.() || window.globalPlayer.play(qIndex, queue, "trending");
        }
        return;
      }
      currentId = id;
