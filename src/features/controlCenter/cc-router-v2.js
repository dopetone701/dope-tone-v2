// CC V9.6 FINAL - AUDIENCES + SUPPORT + DROP ZONE - PURE SVG - V2 INJECT ALL
export async function mountCC(fullPath, viewEl){
  ...
  rightEl.innerHTML = `
    ...
    <nav style="padding:12px 8px; display:flex; flex-direction:column; gap:2px;">
      <button data-cc="overview" class="cc-item ${route==='overview'?'active':''}">
        <span style="width:20px;height:20px;display:grid;place-items:center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M7 14l3-3 3 3 5-5"/></svg></span>Overview
      </button>
      <button data-cc="beats" class="cc-item ${route==='beats'?'active':''}">
        <span style="width:20px;height:20px;display:grid;place-items:center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></span>Beats
      </button>
      <button data-cc="audiences" class="cc-item ${route==='audiences'?'active':''}">
        <span style="width:20px;height:20px;display:grid;place-items:center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></span>Audiences
      </button>
      <button data-cc="tickets" class="cc-item ${route==='tickets'?'active':''}">
        <span style="width:20px;height:20px;display:grid;place-items:center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg></span>Support
      </button>
      <button data-cc="dropzone" class="cc-item ${route==='dropzone'?'active':''}">
        <span style="width:20px;height:20px;display:grid;place-items:center"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></span>Drop Zone
      </button>
  ...
  try{
    ...
    if(route==='overview'){
      pageEl.innerHTML = `<div style="display:grid;grid-template-columns:repeat(6,1fr);gap:10px;margin-bottom:12px"><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px"><div style="font-size:9px;color:#6B7280">TOTAL PLAYS</div><div id="totalPlays" style="font-size:18px;font-weight:800;color:#fff;margin-top:6px">0</div></div><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px"><div style="font-size:9px;color:#6B7280">DOWNLOADS</div><div id="totalDownloads" style="font-size:18px;font-weight:800;color:#fff;margin-top:6px">0</div></div><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px"><div style="font-size:9px;color:#6B7280">CART</div><div id="cartItems" style="font-size:18px;font-weight:800;color:#fff;margin-top:6px">0</div></div><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px"><div style="font-size:9px;color:#6B7280">LIKES</div><div id="totalLikes" style="font-size:18px;font-weight:800;color:#fff;margin-top:6px">0</div></div><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px"><div style="font-size:9px;color:#6B7280">ORDERS</div><div id="totalOrders" style="font-size:18px;font-weight:800;color:#fff;margin-top:6px">0</div></div><div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:12px"><div style="font-size:9px;color:#6B7280">REVENUE</div><div id="totalRevenue" style="font-size:18px;font-weight:800;color:#fff;margin-top:6px">$0</div></div></div><div style="background:#050A14;border:1px solid rgba(255,255,255,.08);border-radius:12px;overflow:hidden"><canvas id="tradeChart" style="width:100%;height:380px;display:block"></canvas><div style="display:grid;grid-template-columns:1fr 1fr;gap:1px;background:rgba(255,255,255,.06)"><div style="background:#050A14;padding:8px 0"><div style="font-size:9px;color:#6B7280;padding:0 12px 6px;letter-spacing:.8px">MOMENTUM</div><canvas id="momentumChart" style="width:100%;height:90px;display:block"></canvas></div><div style="background:#050A14;padding:8px 0"><div style="font-size:9px;color:#6B7280;padding:0 12px 6px;letter-spacing:.8px">CONVERSION</div><canvas id="conversionChart" style="width:100%;height:90px;display:block"></canvas></div></div></div><div id="cc-ranking-wrap" style="margin-top:16px"></div>`;
