...
// ROUTE FIXED FOR V2:../cc-config.js + V2 init/mount + FIT FIX
...
function ensureModalHTML(){
  ...
  if(!modal){
    ...
    console.log('[EDIT] created #editModal div');
  }
  modal.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.85);display:none;align-items:center;justify-content:center;z-index:99999;backdrop-filter:blur(8px);padding:20px 20px 100px 20px;box-sizing:border-box;';

  // FIXED WRAPPER: lift above player
  modal.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,0.85);display:none;align-items:center;justify-content:center;z-index:99999;backdrop-filter:blur(8px);padding:20px 20px 100px 20px;box-sizing:border-box;';
  if(modal.querySelector('.edit-modal-box'))return modal;
  modal.innerHTML=`
  <div class="edit-modal-box" style="background:#0e0e0e;border:1px solid #222;border-radius:16px;width:92%;max-width:760px;max-height:calc(100vh - 130px);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 20px 80px rgba(0,0,0,0.9)">
    <div class="edit-modal-header" style="display:flex;justify-content:space-between;align-items:center;padding:14px 18px;border-bottom:1px solid #222;background:#141414;flex-shrink:0">
      <h3 style="margin:0;font-size:14px;color:#fff"><i class="fa-solid fa-pen-to-square" style="color:#00ff88"></i> Edit Beat — V19 Exact Schema</h3>
    ...
    <div class="edit-modal-body" style="padding:18px;overflow-y:auto;flex:1;">
    ...
    <div class="edit-modal-footer" style="padding:14px 18px;border-top:1px solid #222;display:flex;gap:8px;justify-content:flex-end;background:#141414;flex-shrink:0;border-radius:0 0 16px 16px">
...
export function openEditModal(beat){
  ...
  qs('#editStatus',modal).textContent=`V19 Exact Schema - Editing ${beat.id} - ${beat.title}`;
...
export function closeEditModal(){const m=document.getElementById('editModal');if(m){m.classList.remove('active');m.style.display='none';}document.body.classList.remove('modal-open');document.body.style.overflow='';editingBeat=null;}
...
// V2 EXPORTS
export function init(){ const m=document.getElementById('editModal'); if(m&&!m.querySelector('.edit-modal-box')) ensureModalHTML(); window.addEventListener('cc_edit_beat',(e)=>{const b=allBeats.find(x=>String(x.id)===String(e.detail));if(b)openEditModal(b);}); }
