...
async function loadEmails(){
  ...
  try{
    ...
    // EXPOSE FOR VAULT-EMAILS (UI hook, not logic)
    window.allEmails = allEmails;
    window.getFiltered = getFiltered;
    render();
  }catch(err){
    console.error('[Emails]', err);
    const tb = document.getElementById('emailTableBody');
    if(tb) tb.innerHTML = `<tr><td colspan="4" class="empty-state">Failed: ${err.message}</td></tr>`;
...
// MEANINGFUL SEND FUNCTION WITH creators@
