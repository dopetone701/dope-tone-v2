...
// FIXED:
// • GRID TOGGLE HIDES LIST ROWS COMPLETELY
// • LIST TOGGLE HIDES GRID COMPLETELY
// • GRID USES beats-grid.js
// • PILLS CLICK + DRAG
// • SEARCH + GENRE FILTER WORK IN BOTH VIEWS
// • PRO LIST / GRID STATE
// • SINGLE CLICK = INSTANT PLAY
// • DOUBLE CLICK = INSTANT GO TO BEAT
// ============================================================
...
function getMode(beat) {
  ...
  const m =
    String(
      beat.monetization_mode ||
      beat.monetizationMode ||
      ""
    )
      .toLowerCase()
      .trim();
...
function buyBeat(beat) {
  ...
  window.location.href =
    `licence-page.html?id=${encodeURIComponent(
      beat.id
    )}`;
}
...
async function downloadBeat(
  ...
  try {
    ...
    const url =
      `${DOWNLOAD_API}/api/secure-download/${beat.id}?uid=${encodeURIComponent(
        userId
      )}`;
...
function createDotsMenu(
  ...
  wrap.innerHTML = `
    ...
    <div class="dt-row-menu">
      ...
      <button data-act="goto">
        🎧 Go to beat
      </button>

      <button data-act="playlist">
        ➕ Add to playlist
      </button>

      <button data-act="like">
        ${
          liked
            ? "❤️ Unlike"
            : "🤍 Like beat"
  ...
  menu.onclick = e => {
    ...
    if (
      ...
      const url =
        `${location.origin}/beat.html?id=${beat.id}`;
...
function createListRow(
  ...
  // ==========================================================
  // SINGLE CLICK
  // ANYWHERE ON ROW = PLAY
  // ==========================================================

  row.onclick = e => {
    if (
      e.target.closest(
        ".wave-play,.wave-heat,.wave-buy,.wave-bar,.dt-dots-wrap"
      )
    ) {
      return;
    }
  ...
  // ==========================================================
  // DOUBLE CLICK
  // ANYWHERE ON TRACK CONTENT = GO TO BEAT
  // ==========================================================
  ...
  // ==========================================================
  // IMPORTANT:
  // TITLE / META NO LONGER NAVIGATE ON SINGLE CLICK.
  // They now use the row's instant-play behavior.
  // Double click still reaches row.ondblclick.
  // ==========================================================
...
function setupLazyWaves() {
  ...
  document
    .querySelectorAll(
      "#waveList .wave-row:not([data-wave-observed])"
    )
    .forEach(
      row => {
        row.dataset.waveObserved =
          "1";

        waveObserver.observe(
          row
        );
      }
    );
...
function renderPills() {
  ...
  mount.innerHTML = `
    <div class="dt-header-left">
      ...
      <div
        class="dt-pills-divider"
      ></div>

      <div
        class="dt-pills-scroll"
        id="dtPillsScroll"
      >
...
function setupPlayerSync() {
  ...
  // ==========================================================
  // AUDIO / WAVE SYNC - SPOTIFY PROGRESS
  // ==========================================================
  ...
  (function() {
    ...
    function syncProgress() {
      ...
      const currentRow =
        document.querySelector(
          `.wave-row[data-beat-id="${window.__CURRENT_BEAT__.id}"]`
        );
      ...
      if (currentRow.__wave) {
        try {
          if (
            currentRow.__wave
              .seekTo
          ) {
            currentRow.__wave.seekTo(
              progress
            );
          }
        } catch {}
...
.dt-view-toggle{
  ...
  align-items:center!important;
  height:32px!important;
  background:rgba(10,25,49,.9)!important;
...
.dt-view-btn.active{
  ...
  color:#fff!important;
  box-shadow:0 0 12px rgba(255,30,60,.4)!important
}
.filter-btn{
  height:28px!important;
  min-height:28px!important;
  ...
  border-color:rgba(255,30,60,.25)!important;
  color:#fff!important;
  font-weight:700!important;
  border-radius:999px!important;
}

/* CONTENT BELOW BAR HAS PADDING - BAR IS FULL WIDTH */
#waveList, #gridContainer{
...
#beatsListSentinel, #gridSentinel{
  width:100%;
  height:1px;
}

/* DOTS + MENU + DROPDOWN + BUY */
.wave-dots{
  width:32px;
  height:32px;
  border-radius:8px;
...
.dt-genre-dropdown button[data-g]{
  width:100%;
  text-align:left;
  padding:10px 12px;
  border-radius:8px;
...
.dt-genre-dropdown button[data-g].active,
.dt-genre-dropdown button[data-g]:hover{
  background:rgba(255,30,60,.12);
  color:#FF1E3C
}
.dt-genre-close{
...
/* ===== SPOTIFY ACTIVE TRACK ===== */
.wave-row.is-active{
  background:rgba(255,30,60,.08)!important;
  border-color:rgba(255,30,60,.35)!important;
  box-shadow:
    0 0 0 1px rgba(255,30,60,.15) inset,
    0 4px 20px rgba(0,0,0,.3)!important;
}
.wave-row.is-playing .wave-title{
  color:#FF1E3C!important;
}
.wave-row.is-playing .wave-cover-wrap{
  box-shadow:0 0 0 2px #FF1E3C!important;
}
.wave-row.is-playing .wave-play{
  opacity:1!important;
...
/* WAVE PROGRESS BAR LIKE SPOTIFY */
.wave-bar{
  flex:1!important;
