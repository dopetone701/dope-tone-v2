// ============================================================
// DOPE TONE — SIMILAR VYBS
// FULL LENGTH — COVER FIX + SMOOTH DRAG ONLY
// FIXED:
// - Wheel scroll REMOVED
// - Only drag + momentum kept
// - Vertical page scroll never blocked
// - Failed/blinking covers fixed
// - CORS image issue fixed
// ============================================================
...
function normalizeCover(value) {
  ...
  if (
    ...
    raw.includes('/object/public/') ||
    raw.includes('/object/sign/')
  ) return '/' + raw.replace(/^\/+/, '');
...
// ============================================================
// GET ALL POSSIBLE COVER SOURCES
// ============================================================
...
function getCoverCandidates(beat) {
  ...
  const possible = [
    beat.cover_url, beat.coverUrl, beat.cover,
    beat.cover_image, beat.coverImage,
    beat.artwork_url, beat.artworkUrl, beat.artwork, beat.artwork_image,
    beat.image_url, beat.imageUrl, beat.image,
    beat.thumbnail_url, beat.thumbnailUrl, beat.thumbnail,
    beat.photo_url, beat.photo,
    beat.img_url, beat.img,
    beat.preview_image, beat.previewImage,
    beat.cover_path, beat.coverPath,
    beat.artwork_path, beat.artworkPath,
...
function scoreSimilar(target, candidate, stats) {
  ...
  if (tg && cg) {
    if (tg === cg) score += 40;
    else if (tg.includes(cg) || cg.includes(tg)) score += 20;
  }
  ...
  if (bpmDiff <= 3) score += 22;
  else if (bpmDiff <= 7) score += 14;
...
function getSimilarBeats(seedBeats) {
  ...
  return all
   .filter(beat => beat && beat.id &&!seedIds.has(String(beat.id)))
   .map(beat => ({ beat, score: scoreSimilar(seedMeta, beat, stats) }))
...
function setupCoverImage(img) {
  ...
  const tryNext = () => {
    ...
    if (index >= candidates.length) {
      finished = true;
      img.onerror = null;
      if (img.getAttribute('src')!== FALLBACK_COVER) img.src = FALLBACK_COVER;
      return;
    }
    const next = candidates[index];
    if (!next) { tryNext(); return; }
  ...
  img.onload = () => {
    finished = true;
    img.onerror = null;
    const button = img.parentElement?.querySelector('.st-play');
    if (button) button.style.background = 'linear-gradient(145deg,#FF1E3C,#B90025)';
  };
...
export function renderSimilarTracks(seedBeats, mountId = 'similarMount') {
    ...
    <div class="similar-wrap">
      ...
      <div class="similar-scroll" id="similarScroll" aria-label="Similar tracks">
        ${similar.map((beat, index) => {
          ...
          return `
              <div class="st-card" data-idx="${index}" data-id="${escapeHTML(beat.id)}" tabindex="0" role="button" aria-label="Play ${title}">
                <div class="st-cover">
                  <img src="${escapeHTML(cover)}" data-cover-index="0" data-cover-candidates="${candidates}" alt="" draggable="false" loading="lazy" decoding="async">
                  <button class="st-play" type="button" data-id="${escapeHTML(beat.id)}" aria-label="Play ${title}">
                    <span class="st-icon">${PLAY_SVG}</span>
                  </button>
                  <div class="st-eq"><span></span><span></span></div>
                </div>
                <div class="st-title">${title}</div>
              </div>
            `;
        }).join('')}
        <div class="st-card more-card" id="similarMore" tabindex="0" role="button" aria-label="View all beats">
          <div class="st-cover more-cover">
            <div class="more-grid" aria-hidden="true"><i></i><i></i><i></i></div>
          </div>
          <div class="st-title">View All</div>
        </div>
      </div>
    </div>
  `;
  ...
  scroller.addEventListener('pointerleave', pointerUp);

  // ==========================================================
  // COVER INITIALIZATION
  // ==========================================================

  mount.querySelectorAll('.st-cover img').forEach(img => {
    setDefaultPlayColor(img);
    setupCoverImage(img);
  });
  ...
  const sync = () => {
    ...
    mount.querySelectorAll('.st-card:not(.more-card)').forEach(card => {
      ...
      const icon = card.querySelector('.st-icon');
      if (icon) icon.innerHTML = isPlaying? PAUSE_SVG : PLAY_SVG;
    });
  };

  sync();
  document.addEventListener('trackChange', sync);
  document.addEventListener('playerPlay', sync);
  document.addEventListener('playerPause', sync);
  document.addEventListener('dt:listSwitch', sync);
  ...
  const playBeat = beat => {
    ...
    if (sameTrack && audio) {
      if (audio.paused) {
        if (typeof window.DTPlayer?.play === 'function') window.DTPlayer.play();
        else audio.play().catch(() => {});
      } else {
        if (typeof window.DTPlayer?.pause === 'function') window.DTPlayer.pause();
        else audio.pause();
      }
      sync();
      return;
    ...
    window.__CURRENT_BEATS__ = similar;
    const index = similar.findIndex(item => String(item?.id) === id);
    window.__CURRENT_INDEX__ = index >= 0? index : 0;
    window.__CURRENT_BEAT__ = beat;

    localStorage.setItem('dt_list_v2', 'similar');
    localStorage.setItem('dt_index_v2', String(window.__CURRENT_INDEX__));
    localStorage.setItem('dt_queue_v2', JSON.stringify(similar));

    if (typeof window.DTPlayer?.setQueue === 'function') {
      window.DTPlayer.setQueue(similar, window.__CURRENT_INDEX__, true);
    } else if (typeof window.DTPlayTrack === 'function') {
      window.DTPlayTrack(beat, true);
    } else if (typeof window.DTPlayer?.playTrack === 'function') {
      window.DTPlayer.playTrack(beat, true);
    }
  ...
  if (more) {
    const goBeats = () => {
      if (typeof window.navigateTo === 'function') window.navigateTo('/beats');
      else location.hash = '#/beats';
    };
